...
constexpr qsizetype MaxAccessLinkLength = 8192;

} // namespace

QString encodeAccessInvitation(const AccessInvitation& invitation) {
    if (!SignalingClient::validServerUrl(invitation.server) ||
        !security::validKey(invitation.authority) ||
        security::unbase64(invitation.token, 32).isEmpty()) {
        return {};
    }

    const auto encoded = signaling_protocol::EnvelopeCodec::encode(
        {signaling_protocol::ProtocolVersion, "access.invitation", std::nullopt,
...
std::optional<AccessInvitation> decodeAccessInvitation(const QString& text) {
    ...
    if (!text.startsWith(prefix) || text.size() > MaxAccessLinkLength) {
        return std::nullopt;
    }

    const auto decoded = signaling_protocol::EnvelopeCodec::decode(
        security::unbase64(text.sliced(prefix.size())));
    const auto* value = std::get_if<signaling_protocol::Envelope>(&decoded);
    if (!value || value->type != "access.invitation" || value->requestId || value->body.size() != 3) {
        return std::nullopt;
    }

    AccessInvitation invitation{QUrl(value->body.value("server").toString(), QUrl::StrictMode),
                                value->body.value("authority").toString(),
                                value->body.value("token").toString()};
    if (encodeAccessInvitation(invitation).isEmpty()) {
        return std::nullopt;
    }
    return invitation;
}
...
QJsonObject decodeAccessGrant(const QString& text) {
    ...
    return security::verifyGrant(grant, grant.value("authority").toString(),
                                 grant.value("subject").toString())
               ? grant
               : QJsonObject{};
}
