...
namespace {
bool initialized() { static const bool ready = sodium_init() >= 0; return ready; }
...
bool validKey(const QString& hex) {
    ...
    for (const auto c : hex) if (!(c >= '0' && c <= '9') && !(c >= 'a' && c <= 'f')) return false;
    return hex != QString(64, '0');
}
QString identityId(const QString& key) {
    if (!validKey(key)) return {};
    return QString::fromLatin1(QCryptographicHash::hash(QByteArray::fromHex(key.toLatin1()), QCryptographicHash::Sha256).toHex());
}
...
QByteArray unbase64(const QString& text, int expectedSize) {
    const auto bytes = QByteArray::fromBase64(text.toLatin1(), QByteArray::Base64UrlEncoding | QByteArray::AbortOnBase64DecodingErrors);
    if (base64(bytes) != text || (expectedSize && bytes.size() != expectedSize)) return {};
    return bytes;
}
QString randomToken() {
...
bool verify(const QString& key, const QByteArray& message, const QString& signature) {
    if (!initialized() || !validKey(key)) return false;
    const auto pk = QByteArray::fromHex(key.toLatin1());
    const auto sig = unbase64(signature, crypto_sign_BYTES);
    return sig.size() == crypto_sign_BYTES && crypto_sign_verify_detached(
...
SigningKey::~SigningKey() { sodium_memzero(secret_.data(), secret_.size()); }
std::shared_ptr<SigningKey> SigningKey::load(const QString& path) {
    ...
    if (object.value("v").toInt(-1) != 1 || seed.size() != crypto_sign_SEEDBYTES) return {};
    auto key = std::shared_ptr<SigningKey>(new SigningKey);
    crypto_sign_seed_keypair(key->public_.data(), key->secret_.data(), reinterpret_cast<const unsigned char*>(seed.constData()));
    sodium_memzero(seed.data(), size_t(seed.size()));
    if (object.value("publicKey").toString() != key->publicKey()) return {};
    return key;
}
std::shared_ptr<SigningKey> SigningKey::create(const QString& path) {
    if (!initialized() || path.isEmpty()) return {};
    QLockFile creationLock(path + ".lock");
    if (!creationLock.tryLock(0) || QFile::exists(path)) return {};
    auto key = std::shared_ptr<SigningKey>(new SigningKey);
    ...
    const bool saved = writePrivateJson(path, {{"v", 1}, {"publicKey", key->publicKey()}, {"seed", base64(seed)}});
    sodium_memzero(seed.data(), size_t(seed.size()));
    return saved ? key : nullptr;
}
QString SigningKey::publicKey() const {
    return QString::fromLatin1(QByteArray(reinterpret_cast<const char*>(public_.data()), public_.size()).toHex());
}
QString SigningKey::sign(const QByteArray& message) const {
    ...
    if (crypto_sign_detached(reinterpret_cast<unsigned char*>(signature.data()), nullptr,
        reinterpret_cast<const unsigned char*>(message.constData()), message.size(), secret_.data()) != 0) return {};
    return base64(signature);
}
QJsonObject issueGrant(const SigningKey& authority, const QString& subject) {
    if (!validKey(subject)) return {};
    const auto root = authority.publicKey();
    return {{"v", 1}, {"authority", root}, {"subject", subject}, {"role", "member"},
...
bool verifyGrant(const QJsonObject& grant, const QString& authority, const QString& subject) {
    return grant.size() == 5 && grant.value("v").toInt(-1) == 1 &&
        grant.value("authority").toString() == authority && grant.value("subject").toString() == subject &&
...
bool writePrivateJson(const QString& path, const QJsonObject& object) {
    QSaveFile file(path);
    if (!file.open(QIODevice::WriteOnly) || !file.setPermissions(QFile::ReadOwner | QFile::WriteOwner)) return false;
